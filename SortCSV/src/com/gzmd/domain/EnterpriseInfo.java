package com.gzmd.domain;

public class EnterpriseInfo {
	
	/**
	 * 企业信息实体类,38+6个字段
	 */
	private String NBXH;//*内部序号
	private String ZCH;//注册号
	private String QYMC;//*企业名称
	private String ZS;//经营场所
	private String FDDBR;//法定代表人
	private double ZCZB;//*注册资本
	private String BZ;//注册资本（金）币种
	private String QYLX;//企业类型
	private String LXDH;//联系电话
	private String JYFW;//经营范围
	private String CLRQ;//成立日期
	private String NJJG;//年检结果
	private String NJND;//年检年度
	private String DJJG;//登记机关
	private String JYFS;//经营方式
	private String YYRQ1;//营业日期起
	private String YYRQ2;//营业日期截止
	private String XYDJ;//信用等级
	private String DJRQ;//定级日期
	private String SFHMD;//是否列入黑名单
	private String QYBZ;//企业标示
	private String BY1;//备用一(特殊行业标志)
	private String BY2;//备用二(迁入备注
	private String BY3;//备用三(原注册号)
	private String MCLX;//备用三(原注册号)
	private String HYDM;//行业代码
	private String GXDW;//管辖单位
	private String QZJYFW;//前置经营项目
	private String YBJYFW;//一般经营项目
	private String DZYX;//地址邮箱
	private String LCBZ;//流程标志
	private String FXD;//风险度
	private String OPTYPE;//经营类别
	private String HZRQ;//*核准日期
	private String ZXRQ;//注销日期
	private String DXRQ;//吊销日期
	private String VALI;//有效标志
	private String HYML;//行业门类
	private String UNISCID;//行业门类
	private String YIEDISTRICT;//行业门类
	private String PROLOC;//行业门类
	private String INSERT_TIME;//行业门类
	private String UPDATE_TIME;//行业门类
	private String ZSXZQH;//行业门类
	
	/**
	 * get、set方法
	 * 
	 */
	public String getNBXH() {
		return NBXH;
	}
	public void setNBXH(String nBXH) {
		NBXH = nBXH;
	}
	public String getZCH() {
		return ZCH;
	}
	public void setZCH(String zCH) {
		ZCH = zCH;
	}
	public String getQYMC() {
		return QYMC;
	}
	public void setQYMC(String qYMC) {
		QYMC = qYMC;
	}
	public String getZS() {
		return ZS;
	}
	public void setZS(String zS) {
		ZS = zS;
	}
	public String getFDDBR() {
		return FDDBR;
	}
	public void setFDDBR(String fDDBR) {
		FDDBR = fDDBR;
	}
	public double getZCZB() {
		return ZCZB;
	}
	public void setZCZB(double zCZB) {
		ZCZB = zCZB;
	}
	public String getBZ() {
		return BZ;
	}
	public void setBZ(String bZ) {
		BZ = bZ;
	}
	public String getQYLX() {
		return QYLX;
	}
	public void setQYLX(String qYLX) {
		QYLX = qYLX;
	}
	public String getLXDH() {
		return LXDH;
	}
	public void setLXDH(String lXDH) {
		LXDH = lXDH;
	}
	public String getJYFW() {
		return JYFW;
	}
	public void setJYFW(String jYFW) {
		JYFW = jYFW;
	}
	public String getCLRQ() {
		return CLRQ;
	}
	public void setCLRQ(String cLRQ) {
		CLRQ = cLRQ;
	}
	public String getNJJG() {
		return NJJG;
	}
	public void setNJJG(String nJJG) {
		NJJG = nJJG;
	}
	public String getNJND() {
		return NJND;
	}
	public void setNJND(String nJND) {
		NJND = nJND;
	}
	public String getDJJG() {
		return DJJG;
	}
	public void setDJJG(String dJJG) {
		DJJG = dJJG;
	}
	public String getJYFS() {
		return JYFS;
	}
	public void setJYFS(String jYFS) {
		JYFS = jYFS;
	}
	public String getYYRQ1() {
		return YYRQ1;
	}
	public void setYYRQ1(String yYRQ1) {
		YYRQ1 = yYRQ1;
	}
	public String getYYRQ2() {
		return YYRQ2;
	}
	public void setYYRQ2(String yYRQ2) {
		YYRQ2 = yYRQ2;
	}
	public String getXYDJ() {
		return XYDJ;
	}
	public void setXYDJ(String xYDJ) {
		XYDJ = xYDJ;
	}
	public String getDJRQ() {
		return DJRQ;
	}
	public void setDJRQ(String dJRQ) {
		DJRQ = dJRQ;
	}
	public String getSFHMD() {
		return SFHMD;
	}
	public void setSFHMD(String sFHMD) {
		SFHMD = sFHMD;
	}
	public String getQYBZ() {
		return QYBZ;
	}
	public void setQYBZ(String qYBZ) {
		QYBZ = qYBZ;
	}
	public String getBY1() {
		return BY1;
	}
	public void setBY1(String bY1) {
		BY1 = bY1;
	}
	public String getBY2() {
		return BY2;
	}
	public void setBY2(String bY2) {
		BY2 = bY2;
	}
	public String getBY3() {
		return BY3;
	}
	public void setBY3(String bY3) {
		BY3 = bY3;
	}
	public String getMCLX() {
		return MCLX;
	}
	public void setMCLX(String mCLX) {
		MCLX = mCLX;
	}
	public String getHYDM() {
		return HYDM;
	}
	public void setHYDM(String hYDM) {
		HYDM = hYDM;
	}
	public String getGXDW() {
		return GXDW;
	}
	public void setGXDW(String gXDW) {
		GXDW = gXDW;
	}
	public String getQZJYFW() {
		return QZJYFW;
	}
	public void setQZJYFW(String qZJYFW) {
		QZJYFW = qZJYFW;
	}
	public String getYBJYFW() {
		return YBJYFW;
	}
	public void setYBJYFW(String yBJYFW) {
		YBJYFW = yBJYFW;
	}
	public String getDZYX() {
		return DZYX;
	}
	public void setDZYX(String dZYX) {
		DZYX = dZYX;
	}
	public String getLCBZ() {
		return LCBZ;
	}
	public void setLCBZ(String lCBZ) {
		LCBZ = lCBZ;
	}
	public String getFXD() {
		return FXD;
	}
	public void setFXD(String fXD) {
		FXD = fXD;
	}
	public String getOPTYPE() {
		return OPTYPE;
	}
	public void setOPTYPE(String oPTYPE) {
		OPTYPE = oPTYPE;
	}
	public String getHZRQ() {
		return HZRQ;
	}
	public void setHZRQ(String hZRQ) {
		HZRQ = hZRQ;
	}
	public String getZXRQ() {
		return ZXRQ;
	}
	public void setZXRQ(String zXRQ) {
		ZXRQ = zXRQ;
	}
	public String getDXRQ() {
		return DXRQ;
	}
	public void setDXRQ(String dXRQ) {
		DXRQ = dXRQ;
	}
	public String getVALI() {
		return VALI;
	}
	public void setVALI(String vALI) {
		VALI = vALI;
	}
	public String getHYML() {
		return HYML;
	}
	public void setHYML(String hYML) {
		HYML = hYML;
	}
	public String getUNISCID() {
		return UNISCID;
	}
	public void setUNISCID(String uNISCID) {
		UNISCID = uNISCID;
	}
	public String getYIEDISTRICT() {
		return YIEDISTRICT;
	}
	public void setYIEDISTRICT(String yIEDISTRICT) {
		YIEDISTRICT = yIEDISTRICT;
	}
	public String getPROLOC() {
		return PROLOC;
	}
	public void setPROLOC(String pROLOC) {
		PROLOC = pROLOC;
	}
	public String getINSERT_TIME() {
		return INSERT_TIME;
	}
	public void setINSERT_TIME(String iNSERT_TIME) {
		INSERT_TIME = iNSERT_TIME;
	}
	public String getUPDATE_TIME() {
		return UPDATE_TIME;
	}
	public void setUPDATE_TIME(String uPDATE_TIME) {
		UPDATE_TIME = uPDATE_TIME;
	}
	public String getZSXZQH() {
		return ZSXZQH;
	}
	public void setZSXZQH(String zSXZQH) {
		ZSXZQH = zSXZQH;
	}
	/**
	 * 构造函数
	 */
	public EnterpriseInfo(){
		
	}
	
	public EnterpriseInfo(String nBXH, String zCH, String qYMC, String zS, String fDDBR, double zCZB, String bZ,
			String qYLX, String lXDH, String jYFW, String cLRQ, String nJJG, String nJND, String dJJG, String jYFS,
			String yYRQ1, String yYRQ2, String xYDJ, String dJRQ, String sFHMD, String qYBZ, String bY1, String bY2,
			String bY3, String mCLX, String hYDM, String gXDW, String qZJYFW, String yBJYFW, String dZYX, String lCBZ,
			String fXD, String oPTYPE, String hZRQ, String zXRQ, String dXRQ, String vALI, String hYML, String uNISCID,
			String yIEDISTRICT, String pROLOC, String iNSERT_TIME, String uPDATE_TIME, String zSXZQH) {
		super();
		NBXH = nBXH;
		ZCH = zCH;
		QYMC = qYMC;
		ZS = zS;
		FDDBR = fDDBR;
		ZCZB = zCZB;
		BZ = bZ;
		QYLX = qYLX;
		LXDH = lXDH;
		JYFW = jYFW;
		CLRQ = cLRQ;
		NJJG = nJJG;
		NJND = nJND;
		DJJG = dJJG;
		JYFS = jYFS;
		YYRQ1 = yYRQ1;
		YYRQ2 = yYRQ2;
		XYDJ = xYDJ;
		DJRQ = dJRQ;
		SFHMD = sFHMD;
		QYBZ = qYBZ;
		BY1 = bY1;
		BY2 = bY2;
		BY3 = bY3;
		MCLX = mCLX;
		HYDM = hYDM;
		GXDW = gXDW;
		QZJYFW = qZJYFW;
		YBJYFW = yBJYFW;
		DZYX = dZYX;
		LCBZ = lCBZ;
		FXD = fXD;
		OPTYPE = oPTYPE;
		HZRQ = hZRQ;
		ZXRQ = zXRQ;
		DXRQ = dXRQ;
		VALI = vALI;
		HYML = hYML;
		UNISCID = uNISCID;
		YIEDISTRICT = yIEDISTRICT;
		PROLOC = pROLOC;
		INSERT_TIME = iNSERT_TIME;
		UPDATE_TIME = uPDATE_TIME;
		ZSXZQH = zSXZQH;
	}
}
