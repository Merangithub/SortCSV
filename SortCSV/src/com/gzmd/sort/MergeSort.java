...
public class MergeSort {
	...
	private void merge(Object[] oldArr, int left, int mid, int right, Object[] tempArr, int column) {
		...
		case 1:
			...
			while ((i <= mid) && (j <= right)) {
				if (Long.parseLong(((EnterpriseInfo) oldArr[i]).getNBXH()) <= Long
						.parseLong(((EnterpriseInfo) oldArr[j]).getNBXH())) {
					tempArr[t++] = oldArr[i++];
				} else {
					tempArr[t++] = oldArr[j++];
				}
		...
		case 4:
			...
			while ((i <= mid) && (j <= right)) {
				if ((cnu.compareChinese(((EnterpriseInfo) oldArr[i]).getQYMC(),
						((EnterpriseInfo) oldArr[j]).getQYMC())) <= 0) {
					tempArr[t++] = oldArr[i++];
				} else {
					tempArr[t++] = oldArr[j++];
				}
